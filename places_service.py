...
class PlacesService:
    ...
    def places_get_view(
        ...
        collapsed_node_id: str | None = None,
    ) -> dict[str, Any]:
        ...
        if selected_node_id in nodes:
            resolved_selected = selected_node_id
        else:
    ...
    def _render_node(self, nodes: dict[str, PlaceNode], node_id: str, selected_node_id: str, depth: int, collapsed_node_id: str | None = None) -> str:
        ...
        is_open = (node_id in selected) and (node_id != collapsed_node_id)
        icon = {
            "country": "🌍",
            "region": "🗺️",
            "city": "🏙️",
        ...
        if is_active and node.children:
            if collapsed_node_id == node_id:
                href = f"?node={quote(node_id, safe='')}"
            else:
                href = f"?node={quote(node_id, safe='')}&collapse={quote(node_id, safe='')}"
        else:
